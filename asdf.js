...
<li>
  <pre>
    ...
    for(i = 0; i < myDiv.children.length; i++) {
      myDiv.children[i].addEventListener("click", function(){
        myDiv.children[i].className = myDiv.children[i].className + "clickedStyle";
      });
    }

    // After refactor, no jQuery:
    for(i = 0; i < myDiv.children.length; i++) {
      var child = myDiv.children[i];
      child.addEventListener("click", function(){
        child.className = child.className + "clickedStyle";
      });
    }

    // After refactor, using jQuery:
  ...
  <pre>
    function startScroll () {
      setInterval(function () {
        var scroll =  $("h1.scroll-header"),
        text = scroll.text(),
        first = text.splice(0,1);
        scroll.text(text+first);
      }, 500);
    }
  </pre>
</li>
